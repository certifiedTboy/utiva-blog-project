...
import { PenLine, Eye, MessageCircle, Heart, Edit, Trash2, Plus, FileText, Clock } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your posts and track your impact</p>
          </div>
          <Link href="/write">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          <StatCard label="Total Views" value={totalViews} icon={Eye} color="bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400" />
          <StatCard label="Comments" value={totalComments} icon={MessageCircle} color="bg-green-50 text-green-600 dark:bg-green-950 dark:text-green-400" />
          <StatCard label="Reactions" value={totalReactions} icon={Heart} color="bg-red-50 text-red-600 dark:bg-red-950 dark:text-red-400" />
          <StatCard label="Published" value={published} icon={PenLine} color="bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400" />
        </div>

        <div className="bg-card border border-card-border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <h2 className="font-serif text-xl font-semibold text-foreground">Your Posts</h2>
          ...
          {posts.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <Link href="/write">
                <Button className="mt-4 gap-2" data-testid="button-write-first"><Plus className="w-4 h-4" /> Write Now</Button>
              </Link>
            </div>
          ) : (
                ...
                <motion.div key={post.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1"><Heart className="w-3 h-3" />{post.reactionCount}</span>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link href={`/write/${post.id}`}>
                      <Button size="sm" variant="ghost" className="gap-1" data-testid={`button-edit-post-${post.id}`}>
