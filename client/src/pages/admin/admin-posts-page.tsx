    ...
    <div className="min-h-screen pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="font-serif text-3xl font-semibold text-foreground">Manage Posts</h1>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            placeholder="Search posts..." data-testid="input-search-posts"
        ...
        <div className="bg-card border border-card-border rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-border flex justify-between items-center">
            <span className="text-sm text-muted-foreground">{filtered.length} posts total</span>
          </div>
          {pagePosts.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {pagePosts.map((post, i) => (
                <motion.div key={post.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  className="px-5 py-3 flex items-center gap-4 hover:bg-muted/30 transition-colors group" data-testid={`row-admin-post-${post.id}`}>
                  {post.coverImage && <img src={post.coverImage} alt={post.title} className="w-10 h-10 rounded object-cover flex-shrink-0" />}
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">by {post.authorName} · <Eye className="w-3 h-3 inline" /> {post.viewCount}</p>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link href={`/blog/${post.slug}`}><Button size="sm" variant="ghost"><Eye className="w-3.5 h-3.5" /></Button></Link>
                    <Link href={`/write/${post.id}`}><Button size="sm" variant="ghost"><Edit className="w-3.5 h-3.5" /></Button></Link>
                    <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => handleDelete(post.id, post.title)} data-testid={`button-delete-post-${post.id}`}><Trash2 className="w-3.5 h-3.5" /></Button>
                  </div>
                </motion.div>
              ))}
            </div>
