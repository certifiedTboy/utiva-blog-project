...
export class AuthControllers {
     ...
     */
    static async loginUserWithEmail(req, res, next) {
        try {
     ...
     * @param {NextFunction} next - The Express next middleware function.
     */
    static async loginUserWithGithub(req, res, next) {
        try {
            const { code } = req.query;
            const result = await AuthServices.githubLogin(code);
            ResponseHandler.auth(res, 201, "user created successfully", result);
        }
        catch (error) {
            next(error);
    ...
    static async requestPasswordReset(req, res, next) {
        try {
            ...
            ResponseHandler.ok(res, 200, "email for password reset has been sent to your email", result);
        }
        catch (error) {
            next(error);
        }
    ...
    static async getNewAccessToken(req, res, next) {
        try {
            ...
            ResponseHandler.auth(res, 201, "New access token generated successfully", result);
        }
        catch (error) {
            next(error);
        }
